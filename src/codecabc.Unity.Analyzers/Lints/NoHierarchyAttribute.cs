...
public class NoHierarchyAttribute : DiagnosticAnalyzer
{
	internal static readonly DiagnosticDescriptor Rule = new(
		id: "ACR0006",
		title: Strings.NoHierarchyAttribute,
		messageFormat: Strings.NoHierarchyAttribute,
		category: DiagnosticCategory.Correctness,
		defaultSeverity: DiagnosticSeverity.Info,
		isEnabledByDefault: true,
		description: Strings.NoHierarchyAttribute);
		...
		SyntaxNodeAnalysisContext ctx)
	{
		...
		var isGo = realType.Matches(typeof(UnityEngine.GameObject));
		var isComponent = realType.Matches(typeof(UnityEngine.Component));
		...
		isGo = containingType.Matches(typeof(UnityEngine.GameObject));
		isComponent = containingType.Matches(typeof(UnityEngine.Component));
		...
		if (!isEnumerable)
		{
			return !Utils.HasAnyAttributes(
				symbol,
				typeof(HierarchyValidationAttribute),
				typeof(NoHierarchyValidationAttribute));
		}
		else
		{
			return !Utils.HasAnyAttributes(
				symbol,
				typeof(EnumerableHierarchyValidationAttribute),
				typeof(EnumerableNoHierarchyValidationAttribute));
		}
