...
public class NoRequireComponentsAttribute : DiagnosticAnalyzer
{
	internal static readonly DiagnosticDescriptor Rule = new(
		id: "ACR0005",
		title: Strings.NoRequireComponentsAttribute,
		messageFormat: Strings.NoRequireComponentsAttribute,
		category: DiagnosticCategory.Correctness,
		defaultSeverity: DiagnosticSeverity.Info,
		isEnabledByDefault: true,
		description: Strings.NoRequireComponentsAttribute);
		...
		SyntaxNodeAnalysisContext ctx)
	{
		...
		if (!isEnumerable)
		{
			return !Utils.HasAnyAttributes(
				symbol,
				typeof(RequireComponentsAttribute),
				typeof(NoComponentRequired));
		} 
		else
		{
			return !Utils.HasAnyAttributes(
				symbol,
				typeof(EnumerableRequireComponentsAttribute),
				typeof(EnumerableNoComponentRequired));
		}
	}
}
